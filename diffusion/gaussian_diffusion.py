...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.lambda_prior_preserv = lambda_prior_preserv
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            mask = model_kwargs['y'].get('mask', None)
            if mask is None:
                mask = torch.ones_like(model_output[:, :1, :1, :])  # [B, 1, 1, T]
            ...
            if self.lambda_prior_preserv > 0.:
                
                x_pr_start = model_kwargs['y']['x_pr_start']
                noise_pr = th.randn_like(x_pr_start)
                x_pr_t = self.q_sample(x_pr_start, t, noise=noise_pr, model_kwargs=model_kwargs)
                model_pr_kwargs = {'y': {'text': model_kwargs['y']['x_pr_text'],
                ...
                pr_target = {
                    ModelMeanType.PREVIOUS_X: self.q_posterior_mean_variance(
                        x_start=x_pr_start, x_t=x_pr_t, t=t
                    )[0],
                    ModelMeanType.START_X: x_pr_start,
                    ModelMeanType.EPSILON: noise_pr,
                }[self.model_mean_type]
                
                assert model_pr_output.shape == pr_target.shape == x_pr_start.shape  # [bs, njoints, nfeats, nframes]

                terms["prior_preserv"] = self.masked_l2(pr_target, model_pr_output, model_pr_kwargs['y']['mask']) # mean_flat(rot_mse)
            ...
            terms["loss"] = terms["rot_mse"] + terms.get('vb', 0.) +\
                            ...
                            (self.lambda_prior_preserv * terms.get('prior_preserv', 0.))
