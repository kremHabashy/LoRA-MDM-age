...
def main():
    ...
    if args.edit_mode == 'in_between':
        ...
        model_kwargs['y']['inpainting_mask'] = torch.tensor(humanml_utils.HML_TRAJ_MASK, dtype=torch.bool,
                                                            device=input_motions.device)  # True is lower body data
        model_kwargs['y']['inpainting_mask'] = model_kwargs['y']['inpainting_mask'].unsqueeze(0).unsqueeze(
            -1).unsqueeze(-1).repeat(input_motions.shape[0], 1, input_motions.shape[2], input_motions.shape[3])
    ...
    animations = np.empty(shape=(args.num_samples, args.num_repetitions+1), dtype=object)
    ...
    for sample_i in range(args.num_samples):
        ...
        animations[sample_i, 0] = plot_3d_motion(animation_save_path, skeleton, motion, title=caption,
        ...
        for rep_i in range(args.num_repetitions):
            ...
            animations[sample_i, rep_i+1] = plot_3d_motion(animation_save_path, skeleton, motion, title=caption,
